...
export function ProjectView({ features, setFeatures, openDrawerForId, onDrawerOpened }: Props) {
        ...
        try {
          ...
          setFeatures(prev => prev.map(p => {
            ...
            return {
              ...
              pmOwner:         (d.pmOwner          as string | undefined) ?? p.pmOwner,
              pocEmails:       (d.pocEmails as Record<string, string> | undefined) ?? p.pocEmails,
  ...
  const uniqueStatuses = useMemo(() => {
    const STATUS_DROPDOWN_ORDER = [
      'AB Testing', 'Merged', 'To be merged', 'QA Testing', 'Development', 'Tech Design',
  ...
  const gridTemplateColumns = (() => {
    // Feature is capped at 400px so the Notes column can take the
    ...
    const cols = ['minmax(240px,400px)'];        // Feature
