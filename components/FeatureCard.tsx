...
export function FeatureCard({ feature, syncing, onEdit, onSync }: Props) {
  ...
  return (
    <div className="bg-[var(--card)] border border-[var(--border)] rounded-xl p-5 flex flex-col gap-3 hover:border-[var(--border)] transition-colors">
      {/* Title row */}
      <div className="flex items-start justify-between gap-2">
        <h3 className="text-[var(--foreground)] font-semibold text-base leading-tight">{feature.name}</h3>
        <div className="flex items-center gap-1.5 shrink-0 mt-0.5">
          ...
          <button onClick={() => onEdit(feature)}
            className="text-gray-500 hover:text-[var(--foreground)] transition-colors" title="Edit feature">
            ...
            {feature.tasks.map(task => (
              <div key={task.id} className="flex items-center gap-2">
                <div className={`w-3 h-3 rounded border shrink-0 flex items-center justify-center ${task.completed ? 'bg-emerald-600 border-emerald-600' : 'border-gray-600'}`}>
                  {task.completed && <span className="text-[var(--foreground)] text-[8px]">✓</span>}
                </div>
